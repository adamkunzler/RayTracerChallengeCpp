...
namespace RayTracer
{
	Canvas::Canvas(int lWidth, int lHeight) : width(lWidth), height(lHeight), maxIndex(lWidth* lHeight - 1)
	{
	...
	const void Canvas::fillCanvas(const Color& color)
	{
	...
	const int Canvas::getWidth() { return width; }
	const int Canvas::getHeight() { return height; }

	const Color Canvas::pixelAt(const int index)
	{
		int i = index;
		if (index < 0) i = 0;
	...
	const Color Canvas::pixelAt(const int x, const int y)
	{
		return pixelAt(x + y * getWidth());
	}

	const void Canvas::setPixel(const int index, const Color& color)
	{
		int i = index;
		if (index < 0) i = 0;
		if (index > maxIndex) i = maxIndex;
	...
	const void Canvas::setPixel(const int x, const int y, const Color& color)
	{
	...
	//		
	const void Canvas::toPPM(const std::string& filename)
	{
